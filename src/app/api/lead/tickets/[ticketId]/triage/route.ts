...
import { LeadTriageValidationError, updateTicketTriage } from "@/lib/service-desk/lead-triage";
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    const result = await updateTicketTriage({
    ...
    if (error instanceof LeadTriageValidationError) {
      return validationErrorResponse("Invalid lead triage data.", error.validationError);
    }
